...
export function Scanner({ onReverify }: { onReverify?: () => void } = {}) {
  ...
  async function runScan(e: React.FormEvent) {
    ...
    setData(null);
    try {
      const res = await fetch(`${API_BASE_URL}/api/v1/website/scan`, {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          company_domain: verifiedDomain || null,
          matches_company_domain: matchesCompany,
        }),
      });
  ...
  return (
    <div className="space-y-6">
      {/* Form */}
      <form onSubmit={runScan} className="glass rounded-2xl p-6 md:p-7 space-y-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {verifiedDomain
                  ? <>Scoped to <span className="text-cyan font-medium">{verifiedDomain}</span> and its subdomains.</>
                  : "Authorized website security assessment — basic profile"}
              </p>
            </div>
          </div>
          {verifiedDomain && (
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-xl border border-border bg-secondary px-3 py-2 text-xs font-medium hover:border-cyan/40 hover:text-cyan transition"
            >
        ...
        <div className="grid gap-4 md:grid-cols-[1fr_220px]">
          ...
          <div>
            <label className="text-xs uppercase tracking-wider text-muted-foreground">Scan Profile</label>
            <div className="mt-1.5 grid grid-cols-2 gap-2">
              {(["basic", "advanced"] as ScanProfile[]).map((p) => (
                <button
                  key={p}
                  type="button"
            ...
            <p className="mt-1.5 text-[11px] text-muted-foreground">
              {profile === "basic"
                ? "Fast read-only checks: availability, SSL/TLS, headers, DNS/email."
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="text-sm text-muted-foreground">
                Checking SSL/TLS, security headers, DNS/email security, and risk score.
              </div>
            </div>
          </div>
        </div>
      ...
      {!loading && !error && !data && (
        <div className="glass rounded-2xl p-10 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Enter an authorized website URL to begin your security assessment.
          </p>
        </div>
      )}
        ...
        <div className="space-y-6">
          ...
          <div className="glass rounded-2xl p-5 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div className="text-sm text-muted-foreground">
                Report generated by SecureSight360 Risk Engine. Safe, authorization-based assessment.
              </div>
            </div>
